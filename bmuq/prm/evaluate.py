# Code adapted from https://github.com/GAIR-NLP/ReasonEval/blob/main/codes/mr-math_eval.py
...
class ReasonEval_7B(MistralPreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig) -> None:
        ...
        self.score_head = nn.Linear(
            config.hidden_size, config.score_dimension, bias=config.use_bias
        )
...
def get_results(args, question, reasoning_steps):
    ...
    if args.model_size == "7B":
        adjusted_token_ids = [
            1
        ] + adjusted_token_ids  # Adjusting to recover the first token_ids of the sentences
        adjusted_token_ids = torch.tensor([adjusted_token_ids])
        labeled_token_indices = labeled_token_indices[
            2:
        ]  # Adjusting to skip the first two separator (begining and endding of the problems)
    elif args.model_size == "34B":
        adjusted_token_ids = torch.tensor([adjusted_token_ids])
        labeled_token_indices = labeled_token_indices[
            1:
        ]  # Adjusting to skip the first separator (endding of the problems)
    else:
        raise ValueError(f"Invalid model size: {args.model_size}")
    assert len(labeled_token_indices) == len(reasoning_steps)

    attention_mask = adjusted_token_ids.new_ones(
        adjusted_token_ids.size(), dtype=torch.bool
    )
    # Evaluating reasoning steps using ReasonEval
    with torch.no_grad():
        reasoning_scores = model(adjusted_token_ids, attention_mask)[
    ...
    return (
        ...
        solution_level_validity_scores,
        step_level_redundancy_scores,
        solution_level_redundancy_scores,
    )


def calculate_f1_score(
    correct_true_num, total_true_num, total_false_num, correct_false_num
):
    precision_positive = correct_true_num / total_true_num
    recall_positive = correct_true_num / (
        correct_true_num + total_false_num - correct_false_num
    )
    F1_score_positives = (
        2
        * precision_positive
    ...
    recall_negative = correct_false_num / (
        correct_false_num + total_true_num - correct_true_num
    )
    F1_score_negatives = (
        2
        * precision_negative
...
def get_results_for_invalid_errors():
    test_set_filepath = "./data/reasoneval/mr-math_invalid_errors.json"
    test_dataset = []
    ...
    evaluators = [
        ...
        # "gpt3_5_turbo",
        # "gpt4",
        # "math-shepherd_mistral-7b",
        # "reasoneval_llama2-7b",
        # "reasoneval_wizardmath-7b-v1.0",
        # "reasoneval_mistral-7b",
        # "reasoneval_llemma-7b",
        # "reasoneval_abel-7b-002",
        # "reasoneval_wizardmath-7b-v1.1",
        # "reasoneval_llemma-34b",
    ]
    for evaluator in evaluators:
        score_filepath = (
            "./eval_results/mr-math_invalid_errors/" + evaluator + "_eval_results.json"
        )
    ...
    print(
        "**************invalid errors*********solution level**********macro f1 score*******"
    )
        ...
        if evaluator_name in ["gpt3_5_turbo", "gpt4"]:
            for test_data, score_result in zip(test_dataset, score_results):
                if test_data["model_output_solution_correctness"] == "correct":
                    total_true_num += 1
                    if (
                        score_result["correctness_pred"] == "Correct"
                        or score_result["correctness_pred"] == "correct"
                    ):
                        correct_true_num += 1
                elif test_data["model_output_solution_correctness"] == "wrong":
                    total_false_num += 1
                    if (
                        score_result["correctness_pred"] == "Wrong"
                        or score_result["correctness_pred"] == "wrong"
                    ):
                        correct_false_num += 1
        else:
            for test_data, score_result in zip(test_dataset, score_results):
                if evaluator_name in ["math-shepherd_mistral-7b"]:
                    ...
                    new_score_list = [item[2] for item in score_result["scores"][1:]]
                ...
                if test_data["model_output_solution_correctness"] == "correct":
                    ...
                    total_true_num += 1
                if (
                    test_data["model_output_solution_correctness"] == "wrong"
                    and test_data["model_output_solution_first_error_step"] != "N/A"
                ):
                    total_false_num += 1
                    if solution_level_score < threshold[evaluator_name]:
                        correct_false_num += 1
    ...
    print(
        "**************invalid errors*********step level**********macro f1 score*******"
    )
    for evaluator_name, score_results in score_dict.items():
        correct_true_num = 0
        correct_false_num = 0
        ...
        if evaluator_name in ["roscoe-sa", "roscoe-ss"]:
            ...
            for test_data, score_result in zip(test_dataset, score_results):
                ...
                if test_data["model_output_solution_correctness"] == "correct":
                    ...
                    gt = (
                        [1]
                        * (int(test_data["model_output_solution_first_error_step"]) - 1)
                        + [0]
                        + ["N/A"]
                        * (
                            len(test_data["model_output_step_format"])
                            - int(test_data["model_output_solution_first_error_step"])
                        )
                    )
                ...
                if evaluator_name in ["gpt3_5_turbo", "gpt4"]:
                    if (
                        ...
                        if step_pred != "" and int(step_pred) <= len(
                            test_data["model_output_step_format"]
                        ):
                    ...
                    if evaluator_name in ["math-shepherd_mistral-7b"]:
                        ...
                        raw_scores = [item[2] for item in score_result["scores"][1:]]
                    tag_len = 0
                    # for step_idx, sub_step in enumerate(
                    #     test_data["model_output_step_format"]
                    # ):
                    #     try:
                    #         scores.append(
                    #             min(raw_scores[tag_len : len(sub_step) + tag_len])
                    #         )
                    #     except ValueError:
                    #         print("ValueError encountered in min calculation.")
                    #         print(raw_scores)
                    #         print(tag_len)
                    #         print(len(sub_step))
                    #         print(sub_step)
                    ...
                    pred = [
                        1 if score > threshold[evaluator_name] else 0
                        for score in scores
                    ]
        ...
        if evaluator_name in ["gpt3_5_turbo", "gpt4"]:
            ...
            for test_data, score_result in zip(test_dataset, score_results):
                if evaluator_name in ["math-shepherd_mistral-7b"]:
                    ...
                    pred.append(min([item[2] for item in score_result["scores"][1:]]))
                if (
                    test_data["model_output_solution_correctness"] == "wrong"
                    and test_data["model_output_solution_first_error_step"] != "N/A"
                ):
        ...
        if evaluator_name in ["gpt3_5_turbo", "gpt4", "roscoe-sa", "roscoe-ss"]:
            ...
            for test_data, score_result in zip(test_dataset, score_results):
                ...
                if (
                    ...
                    target.extend(
                        (test_data["model_output_solution_first_error_step"] - 1) * [1]
                        + [0]
                    )
                    pred.extend(
                        scores[: test_data["model_output_solution_first_error_step"]]
                    )
...
def get_results_for_redundant_errors():
    test_set_filepath = "./data/reasoneval/mr-math_redundant_errors.json"
    test_dataset = []
    with open(test_set_filepath) as f:
        for line in f:
            test_dataset.append(json.loads(line))
    ...
    for evaluator in evaluators:
        score_filepath = (
            "./eval_results/mr-math_redundant_errors/"
            + evaluator
            + "_eval_results.json"
        )
        score_results_record = []
    ...
    ### solution-level F1 score
    print(
        "**************redundant errors*********solution level**********macro f1 score*******"
    )
    for evaluator_name, score_results in score_dict.items():
        correct_true_num = 0
        correct_false_num = 0
        ...
        if evaluator_name in ["gpt3_5_turbo", "gpt4"]:
            for test_data, score_result in zip(test_dataset, score_results):
                if 0 not in test_data["rating"]:
                    total_true_num += 1
                    if (
                        score_result["correctness_pred"] == "Absent"
                        or score_result["correctness_pred"] == "absent"
                    ):
                        correct_true_num += 1
                elif 0 in test_data["rating"]:
                    total_false_num += 1
                    if (
                        score_result["correctness_pred"] == "Present"
                        or score_result["correctness_pred"] == "present"
                    ):
                        correct_false_num += 1
        else:
            for test_data, score_result in zip(test_dataset, score_results):
                if evaluator_name in ["math-shepherd_mistral-7b"]:
                    ...
                    new_score_list = [-item[1] for item in score_result["scores"][1:]]
                    solution_level_score = min(new_score_list)
    ...
    print(
        "**************redundant errors*********step level**********macro f1 score*******"
    )
    for evaluator_name, score_results in score_dict.items():
        correct_true_num = 0
        correct_false_num = 0
            ...
            for test_data, score_result in zip(test_dataset, score_results):
                ...
                if evaluator_name in ["gpt3_5_turbo", "gpt4"]:
                    ...
                    if evaluator_name in ["math-shepherd_mistral-7b"]:
                        ...
                        raw_scores = [-item[1] for item in score_result["scores"][1:]]
                    scores = raw_scores
                    assert len(scores) == len(test_data["model_output_step_format"])
                    pred = [
                        1 if score > threshold[evaluator_name] else 0
        ...
        if evaluator_name in ["gpt3_5_turbo", "gpt4"]:
            ...
            for test_data, score_result in zip(test_dataset, score_results):
                if evaluator_name in ["math-shepherd_mistral-7b"]:
                    ...
                    pred.append(min([-item[1] for item in score_result["scores"][1:]]))
                if 0 in test_data["rating"]:
                    target.append(0)
                else:
                    target.append(1)
        ...
        if evaluator_name in ["gpt3_5_turbo", "gpt4", "roscoe-sa", "roscoe-ss"]:
            ...
            for i, j in zip(test_dataset, score_results):
                ...
                if evaluator_name in ["math-shepherd_mistral-7b"]:
                    ...
                    raw_scores = [-item[1] for item in j["scores"][1:]]
                scores = raw_scores
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--error_type", type=str, choices=["invalid", "redundant"], default="invalid"
    )
