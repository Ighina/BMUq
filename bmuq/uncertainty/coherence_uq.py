...
class CoherenceBasedUQ(UncertaintyMethod):
    ...
    def __init__(
        ...
        if coherence_method not in valid_methods:
            raise ValueError(
                f"Invalid coherence method: {coherence_method}. "
    ...
    def _compute_coherence_score(
        self, current_text: str, previous_texts: List[str]
    ) -> float:
        """
        ...
        if self.coherence_method == "mean_cosine_similarity":
            ...
            weights = np.array(
                [
                    self.decay ** (len(previous_texts) - i - 1)
    ...
    def evaluate_step(
        ...
        try:
            ...
            metadata = {
                ...
                "model_name": (
                    self.model._modules["0"].auto_model.name_or_path
    ...
    def batch_evaluate_steps(
        ...
        try:
            ...
            if previous_texts:
                ...
                batch_similarities = (
                    util.cos_sim(curr_embeddings, prev_embeddings).cpu().numpy()
                )

                results = []
                for i, (step, similarities) in enumerate(
                    zip(steps_to_evaluate, batch_similarities)
                ):
                    ...
                    if self.coherence_method == "mean_cosine_similarity":
                        ...
                        weights = np.array(
                            [
                                self.decay ** (len(previous_texts) - j - 1)
...
class RelativeCoherenceBasedUQ(UncertaintyMethod):
    ...
    def __init__(
        ...
        # Validate coherence method
        valid_methods = {"arp_pair", "arp_cos", "arp_std", "segrefree", "silhouette"}
        if coherence_method not in valid_methods:
            raise ValueError(
                f"Invalid coherence method: {coherence_method}. "
                f"Must be one of {valid_methods}"
        ...
        self.control_dataset = control_dataset.sample(frac=1, random_state=seed)[
            col_name
        ].to_list()
    ...
    def _compute_coherence_score(
        self, input_texts: List[str], question: Optional[str]
    ) -> float:
        """
        Compute coherence score between current step and previous steps.
        ...
        if self.precomputed:
            ...
            control_embeddings = self._embed_texts(
                self.control_dataset[: len(input_texts)]
            )

        # Compute similarity scores
        score = self.coherence_scorer(
            embeddings, control_embeddings, **self.coherence_args
        )
        ...
        topic_score = ((topic_score - control_topic_score)) / (
            topic_score + control_topic_score
        )
    ...
    def evaluate_step(
        ...
        return UncertaintyScore(value=confidence, method=self.name, metadata=metadata)
    ...
    def evaluate_path(
        self, reasoning_path: ReasoningPath, question: Optional[str]
    ) -> float:
        """
        Evaluate coherence-based uncertainty for an entire reasoning path.
...
# Factory function for easy instantiation
def create_relative_coherence_uq(
    coherence_method: str = "arp_pair",
    model_name: str = "all-MiniLM-L6-v2",
    add_topic_score: bool = False,
