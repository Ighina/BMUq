        ...
        for _ in range(self.num_samples):
            try:
                response = self.llm.generate(
                    prompt, max_tokens=150, temperature=self.temperature
                )
...
class ConsistencyBasedUQ(UncertaintyMethod):
    ...
    def evaluate_step(
        ...
        try:
            ...
            # Aggregate scores
            overall_score = sum(score for _, score in consistency_scores) / len(
                consistency_scores
            )
            ...
            return UncertaintyScore(
                value=overall_score, method=self.name, metadata=metadata
            )
    ...
    def _check_mathematical_consistency(
        self, question: str, reasoning_path: List[ReasoningStep], step: ReasoningStep
    ) -> float:
...
Provide only a numerical score between 0.0 and 1.0:"""
    ...
    def _check_logical_consistency(
        self, reasoning_path: List[ReasoningStep], step: ReasoningStep
    ) -> float:
        """Check logical consistency with previous steps."""
    ...
    def _has_contradiction(self, content1: str, content2: str) -> bool:
        ...
        contradiction_patterns = [
            (
                r"(\w+)\s*=\s*(\d+)",
                r"\1\s*=\s*(?!\2)\d+",
            ),  # Different values for same variable
    ...
    def _steps_are_connected(self, step1: ReasoningStep, step2: ReasoningStep) -> bool:
        ...
        shared_math_terms = sum(
            1 for term in math_terms if term in content1 and term in content2
        )
...
class MajorityVoteUQ(UncertaintyMethod):
    """
    Majority vote baseline for uncertainty quantification.

    Evaluates uncertainty based on the frequency of the most common answer
    among multiple reasoning paths.
    """

    def __init__(self):
        """Initialize majority vote baseline."""
        super().__init__("majority_vote")
    ...
    def evaluate_step(
        ...
        """Majority vote is not applicable at step level; return neutral score."""
        return UncertaintyScore(
            value=0.5,
            method=self.name,
            metadata={"note": "Majority vote not applicable at step level"},
        )

    def evaluate_path(self, question: str, reasoning_path: ReasoningPath) -> float:
        """Evaluate path confidence based on majority answer frequency."""
        if not reasoning_path.steps:
            return 0.0

        # Extract final answers from all steps
        answers = [step.content.strip().lower() for step in reasoning_path.steps]
        ...
        # Confidence is the proportion of the most common answer
